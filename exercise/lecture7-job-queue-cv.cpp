      ...
      while(!stop) {
        ...
        // my job is to iteratively grab a task from the queue
        {
          ...
          while(queue.empty() && !stop) {
            ...
            // now let's see if we fixed this issue
            // case 1: this worker has aquired the lock and starts the wait
            //         then the main thread will aquire the lock and fire the
            //         stop signal, and notify all, which is fine
            //         cuz this worker will carry on and check the stop condition
            //         again and stop
            //
            //
            // case 2: this worker hasn't even aquired the lock yet, the lock is being
            //         held by the main thread, the main thread will fire the stop signal
            //         and notify all the workers, so when this worker aquires the lock
            //         and check the while condition, it would not even call wait
            cv.wait(lock);
          }
          if(!queue.empty()) {
            task = queue.front();
